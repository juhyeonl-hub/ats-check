...
### 2026-08-15 — 🎉 **v0.1.0 릴리스 완료. §13 완료 정의 전항 충족.**

**https://github.com/juhyeonl-hub/ats-check/releases/tag/v0.1.0**

| DoD 항목 | 상태 |
|---|---|
| 3개 OS 네이티브 바이너리가 GitHub Releases에 있다 | ✅ linux-amd64 / macos-arm64 / windows-amd64 |
| 각 바이너리가 단일 파일이다 | ✅ 사이드카 0 |
| 체크섬이 첨부되어 있다 | ✅ 다운로드 후 3개 검증 |
| `save`한 공고가 배치 결과에서 URL과 함께 나온다 | ✅ |
| 골든 파일 30개 이상이 CI에서 통과 | ✅ 30개 |
| README에 설치 방법과 데모 | ✅ 데모 GIF 포함 |
| ADR 4개 | ✅ **5개** |
| 태그를 푸시하면 릴리스가 자동 생성 | ✅ rc1·rc2·v0.1.0으로 실증 |

**릴리스 검증 (내려받아 실행):** `ats-check 0.1.0` / `GLIBC_2.17` / exit 2·0 정확

**rc를 두 번 돌려서 잡은 것 — 전부 로컬에서는 드러날 수 없던 문제:**
1. `set -u`가 SDKMAN 초기화를 죽여 **Linux 빌드 자체가 실패**
2. `native-image-*.args`를 사이드카로 오인한 **과도한 검사** (Windows)
3. 릴리스 바이너리가 **`0.1.0-SNAPSHOT`**을 보고

**ADR-003 실측 확인:** 컨테이너 빌드가 `GLIBC_2.17`을 요구한다 (목표 2.31, 로컬 빌드 2.34).
RHEL 7까지 커버한다.
